...
export class OracleModule extends RuntimeModule<Record<string, never>> {
  ...
  public async verifyIfRealAmountIsMoreThanTarget(targetAmount: UInt224): Promise<Bool> {
    const realAmount = await this.realAmount.get();
    const isRealAmountMoreThanTarget = targetAmount.lessThanOrEqual(UInt224.from(realAmount.value.toString()));
  ...
  @runtimeMethod() 
  public async verifyReserve(realAmount: UInt224, signature: Signature, targetAmount: UInt224) {
    ...
    const isRealAmountMoreThanTarget = await this.verifyIfRealAmountIsMoreThanTarget(targetAmount);
  ...
  @runtimeMethod() 
  public async verifyReserveForMinting(realAmount: UInt224, targetAmount: UInt224) {
    // set realAmount to the state
    this.realAmount.set(realAmount);

    // Check if real amount is greater or equal to the target amount
    const isRealAmountMoreThanTarget = await this.verifyIfRealAmountIsMoreThanTarget(targetAmount);
    assert(isRealAmountMoreThanTarget, "You have not enough reserves to mint the synthetic asset");
